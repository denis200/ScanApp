      ...
      <View
        ...
        <View style={{ marginLeft: 15 }}>
          <Image
            ...
            style={{ width: 70, height: 70, resizeMode: 'contain' }}></Image>
        </View>
        <View style={{ width: '74%' }}>
...
export default function PayScreen({ route, navigation }) {
  ...
  const BuySomething = (userid) => {
    ...
    fetch(`https://scanappbarcode.azurewebsites.net/SubmitPurchase/${userid}`, {
      method: 'POST',
      headers: {
      ...
      {loading ? (
        ...
        <View>
          ...
          <ScrollView
            ...
            {goodspay.map((good) => (
              <PayGood
                ...
                price={good.price}
                image={good.image}
                quantity={good.quantity}></PayGood>
            ))}
          </ScrollView>
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text style={{ fontSize: 30, marginRight: '5%' }}>{sum.toFixed(2)}</Text>
          </View>
          ...
          <TouchableOpacity
            onPress={() => {
              creditcard ? BuySomething(userid) : Alert.alert('Внимание', 'Добавьте карту!');
            }}
            style={{
